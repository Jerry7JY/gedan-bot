...
if os.path.exists(".env"):
    ...
    print("⚠️ .env файл не найден, используем переменные окружения Railway")
...
Правила площадки:
- Посещение территории лофта возможно только при наличии оригиналов соответствующих документов.
- Каждый участник несет личную ответственность за свои действия, состояние и сохранность своих вещей.
- Пронос алкогольной продукции и еды на территорию лофта запрещен.
...
Каждый тариф — это не просто билет, это твой уникальный опыт и комьюнити!
    ...
    keyboard = [
        ...
        [
            ...
            types.InlineKeyboardButton(text="💼 ВСЕ ТАРИФЫ", callback_data="show_all_tariffs")
        ]
    ]
    markup = types.InlineKeyboardMarkup(inline_keyboard=keyboard)
...
async def process_tariff_type(callback: types.CallbackQuery, state: FSMContext):
    ...
    type_names = {
        ...
        "couple": "💞 <b>ТАРИФЫ ДЛЯ ПАР</b>"
    }
    
    if tariff_type == "male":
        keyboard = [
            [types.InlineKeyboardButton(text="👤 Единоличный - 2000₽", callback_data="tariff_Единоличный")],
            [types.InlineKeyboardButton(text="👥 ЛД - 3500₽", callback_data="tariff_ЛД")],
            [types.InlineKeyboardButton(text="👥👥 Компания друзей - 6500₽", callback_data="tariff_Компания друзей")],
            [types.InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_tariff_types")]
        ]
    elif tariff_type == "female":
        keyboard = [
            [types.InlineKeyboardButton(text="🧍‍♀️ Сильная и независимая - 1500₽", callback_data="tariff_Сильная и независимая")],
            [types.InlineKeyboardButton(text="👭 ЛП - 2500₽", callback_data="tariff_ЛП")],
            [types.InlineKeyboardButton(text="👭👭 Серпентарий - 4500₽", callback_data="tariff_Серпентарий")],
            [types.InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_tariff_types")]
        ]
    elif tariff_type == "couple":
        keyboard = [
            [types.InlineKeyboardButton(text="👩‍❤️‍👨 Инь-янь - 3000₽", callback_data="tariff_Инь-янь")],
            [types.InlineKeyboardButton(text="⬅️ НАЗАД", callback_data="back_to_tariff_types")]
        ]
...
async def show_all_tariffs(callback: types.CallbackQuery, state: FSMContext):
    ...
    all_tariffs_text = """
<b>🎫 ВСЕ ТАРИФЫ НА HALLOWEEN GEDAN PARTY</b>

🦁 <b>ДЛЯ ПАРНЕЙ:</b>
• 👤 Единоличный - 2000₽
• 👥 ЛД - 3500₽ (1750₽/чел)
• 👥👥 Компания друзей - 6500₽ (1625₽/чел)

🌸 <b>ДЛЯ ДЕВУШЕК:</b>
• 🧍‍♀️ Сильная и независимая - 1500₽
• 👭 ЛП - 2500₽ (1250₽/чел)
• 👭👭 Серпентарий - 4500₽ (1125₽/чел)

💞 <b>ДЛЯ ПАР:</b>
• 👩‍❤️‍👨 Инь-янь - 3000₽ (1500₽/чел)
"""
